...
void receiver()
{
  if (Serial.available() > 0) {   // something came across serial
    ...
    receiver_input_channel_4=0;
      ...
      receiver_input_channel_3 *= 10;  // shift left 1 decimal place
      // convert ASCII to integer, add, and shift left 1 decimal place
      receiver_input_channel_3 = ((incomingByte - '0' ) + receiver_input_channel_3);
    }
    while(1) {            // force into a loop until 'n' is received
      incomingByte = Serial.read();
      if (incomingByte == ',') break;   // exit the while(1), we're done receiving
      if (incomingByte == -1) continue;  // if no characters are in the buffer read() returns -1
      receiver_input_channel_4 *= 10;  // shift left 1 decimal place
      // convert ASCII to integer, add, and shift left 1 decimal place
      receiver_input_channel_4 = ((incomingByte - '0' ) + receiver_input_channel_4);
    }
    while(1) {            // force into a loop until 'n' is received
      incomingByte = Serial.read();
      ...
      if (incomingByte == ',') break;   // exit the while(1), we're done receiving
      if (incomingByte == -1) continue;  // if no characters are in the buffer read() returns -1
      receiver_input_channel_2 *= 10;  // shift left 1 decimal place
      // convert ASCII to integer, add, and shift left 1 decimal place
      receiver_input_channel_2 = ((incomingByte - '0' ) + receiver_input_channel_2);
    }
    while(1) {            // force into a loop until 'n' is received
      incomingByte = Serial.read();
      ...
      if (incomingByte == ':') break;   // exit the while(1), we're done receiving
      if (incomingByte == -1) continue;  // if no characters are in the buffer read() returns -1
      receiver_input_channel_1 *= 10;  // shift left 1 decimal place
      // convert ASCII to integer, add, and shift left 1 decimal place
      receiver_input_channel_1 = ((incomingByte - '0' ) + receiver_input_channel_1);
    }
    }
   }
}
...
void setup(){
  ...
  //Setting the accelerometer and gyrometer setoff values
...
//Serial.print("Reached end of setup....");
...
void loop(){
  ...
  //Serial.print("PID Output Pitch:");Serial.print(pid_output_pitch);
  //Serial.print("\nPID Output Roll:");Serial.print(pid_output_roll);
  //Serial.print("\nPID Output Yaw:");Serial.print(pid_output_yaw);
